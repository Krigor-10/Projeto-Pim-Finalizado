...
class MainApp(ctk.CTk):
    ...
    # --- Funções de Envio de Atividade (PROFESSOR) ---
    def abrir_janela_envio_atividade_professor(self):
        """
        Abre a janela para o professor enviar uma atividade
        para sua turma designada.
        """
        # 1. Validar se o professor tem uma turma designada
        turma_professor = self.current_user.get("ID_TURMAS")

        if (not turma_professor) or (str(turma_professor).strip().upper() == "GERAL"):
            messagebox.showwarning(
                "Ação Inválida",
                "Você precisa ter uma turma específica (ex: 'ADS-1A') "
                "associada ao seu perfil para enviar atividades.\n\n"
                "Peça a um Administrador para atualizar seu cadastro.",
                parent=self,
            )
            return

        # 2. Criar a janela (Toplevel)
        self.prof_current_file_path = None

        envio_window = ctk.CTkToplevel(self)
        envio_window.title("Enviar Atividade para Turma")
        envio_window.geometry("500x300")
        envio_window.transient(self)
        ...
        ctk.CTkLabel(
            frame,
            text=f"Professor: {self.current_user.get('NOME', 'N/A')}",
            text_color="gray",
        ).pack(pady=(0, 5))

        ctk.CTkLabel(
            frame,
            text=f"Sua Turma de Destino: {turma_professor}",
            font=("Arial", 14, "bold"),
            text_color="#4CAF50",
        ).pack(pady=(0, 15))
        ...
        ctk.CTkButton(
            ...
            command=lambda: self._enviar_atividade_professor_action(
                envio_window, turma_professor
            ),
            fg_color="green",
        ).pack(pady=20)
    ...
    def _enviar_atividade_professor_action(self, window, turma):
        """Verifica e copia o arquivo para a pasta da turma."""
        ...
        try:
            ...
            pasta_destino_turma = os.path.join(DIR_ATIVIDADES, str(turma))
...
# atualizado em 09/11 ás 18:30 - add função de enviar atividades prof
